...
import { addUser, currentUser, deleteuser, getalluser, getUserByManager, getusers, updateuser } from "store/user/user_action"

const UsersByManagerList = props => {
  ...
  const users = useSelector(state => state.User_reducer.listAll).reverse()
  ...
  useEffect(() => {
    if (users && !users.length) {
      dispatch(getUserByManager(curUser))
      setIsEdit(false)
    }
  }, [dispatch, users])
...
export default withRouter(UsersByManagerList)
